...
class TestPlannerReal(Node):
    ...
    def close_gripper(self, side: str = 'right', duration: float = 1.5):
        ...
        pub = self.right_gripper_pub
        self.get_logger().info(f'{side.capitalize()} gripper -> CLOSED')
        start = time.time()
        while (time.time() - start) < duration:
            pub.publish(msg)
            rclpy.spin_once(self, timeout_sec=0.05)
    ...
    def run_tests(self):
        ...
        # Test 2: Plan only for left arm (kept as planning-only safety check)
        self.get_logger().info('-' * 40)
        ...
        if success1 and success2:
            ...
            # CHANGED: removed left arm execute test
